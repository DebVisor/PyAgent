...
## Problem statement
No GitHub Actions workflow runs `cargo clippy`, `cargo test`, or `cargo audit` for `rust_core/`. Rust regressions are only caught indirectly through Python FFI tests.
