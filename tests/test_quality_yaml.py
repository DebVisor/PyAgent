...
def test_ci_yaml_has_check_job() -> None:
    """ci.yml must define a 'check' job that runs CI checks."""
    with open(".github/workflows/ci.yml", encoding="utf-8") as fh:
        cfg = yaml.safe_load(fh)
    jobs = cfg.get("jobs", {})
    assert "check" in jobs, "'check' job missing from ci.yml"


def test_ci_yaml_check_job_has_install_step() -> None:
    """The check job should include a dependency installation step."""
    with open(".github/workflows/ci.yml", encoding="utf-8") as fh:
        cfg = yaml.safe_load(fh)
    steps = cfg["jobs"]["check"].get("steps", [])
    step_text = " ".join([step.get("run", "") for step in steps]).lower()
    assert "pip install" in step_text, "No install step found in ci.yml check job"
