...
CHECKS = [
    {
        'name': 'bandit',
        'cmd': lambda target, out: ['bandit', '-r', str(target), '-f', 'json', '-o', str(out)],
        'out_suffix': 'bandit.json',
        'install_hint': 'pip install bandit'
    },
    {
        'name': 'semgrep',
        'cmd': lambda target, out: ['semgrep', '--config', 'auto', '--json', '--output', str(out), str(target)],
...
def run_python_only_checks(target: Path) -> dict:
    ...
    for p in target.rglob('*.py'):
        try:
            ...
            findings[str(p)] = [f'parse_error: {e}']
            continue
    ...
    out = OUT_DIR / 'python_only_checks.json'
...
def run_check(check, target: Path) -> tuple[int, str]:
    ...
    if exe:
        ...
        if name == 'bandit':
            cmd = [py, '-m', 'bandit', '-r', str(target), '-f', 'json', '-o', str(out)]
        elif name == 'semgrep':
            # semgrep exposes CLI via semgrep.cli module
            cmd = [py, '-m', 'semgrep.cli', '--config', 'auto', '--json', '--output', str(out), str(target)]
        else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        print('usage: run_static_checks.py <path-to-check>')
        return 2
    target = Path(argv[0])
    ...
    for check in CHECKS:
        ...
        results[check['name']] = {'code': code, 'result': msg}
        print(f"{check['name']}:", code, msg)
    # run lightweight python-only checks regardless of external tool availability
    py_findings = run_python_only_checks(target)
    results['python_only'] = {'code': 0, 'result': f'{len(py_findings)} files flagged', 'files': py_findings}
    print('python_only:', len(py_findings), 'files flagged')
    # write summary
    summary = OUT_DIR / 'summary.json'
    summary.write_text(json.dumps(results, indent=2), encoding='utf-8')
    # Record tool exit codes but do not treat findings as fatal.
