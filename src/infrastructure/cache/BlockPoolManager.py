...
class ARCPolicy:
    ...
    def _evict(self) -> Optional[int]:
        ...
        if len(self._t1) > 0 and (len(self._t1) > self.p or len(self._t2) == 0):
            # Evict from T1
            evicted_id, evicted_block = self._t1.popitem(last=False)
            # Add to B1 ghost
            ...
            # Evict from T2
            evicted_id, evicted_block = self._t2.popitem(last=False)
            # Add to B2 ghost
