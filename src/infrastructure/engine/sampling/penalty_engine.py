...
class PenaltyConfig:
    ...
    bad_words_penalty: float = float("-inf")  # Penalty regarding bad words
...
class PenaltyEngine:
    ...
    def apply_repetition_penalty(
        ...
        """Apply multiplicative repetition penalty regarding token sets."""
    ...
    def apply_frequency_penalty(
        ...
        """Apply additive frequency penalty regarding generated token counts."""
        res: NDArray[np.float32] = logits.copy()
    ...
    def apply_presence_penalty(
        ...
        """Apply additive presence penalty regarding token presence."""
        res: NDArray[np.float32] = logits.copy()
    ...
    def apply_ngram_penalty(
        ...
        """Apply n-gram repetition penalty regarding sequence history."""
        if len(tokens) < n - 1:
            return logits
    ...
    def apply_positional_penalty(
        ...
        """Apply positional penalty regarding distance-based decay."""
        res: NDArray[np.float32] = logits.copy()
        num_t = len(tokens)
    ...
    def apply_bad_words(
        ...
        """Apply bad words blocking regarding safety constraints."""
        res: NDArray[np.float32] = logits.copy()
    ...
    def get_token_stats(self) -> dict[str, Any]:
        ...
        total: int = sum(self.state.token_counts.values())
