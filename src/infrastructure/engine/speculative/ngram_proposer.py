...
class NgramCache:
    ...
    def lookup(self, prefix: list[int]) -> list[int]:
        """Look up positions regarding prefix."""
        with self._lock:
            return list(self._cache.get(tuple(prefix), []))

    def _evict_oldest(self) -> None:
        """Evict oldest entries when cache is full."""
        to_remove = list(filter(lambda k: len(self._cache[k]) == 1, self._cache.keys()))
...
class NgramProposer:
    ...
    def _find_match(self, context: list[int], prefix: list[int], excluded: set[int]) -> NgramMatch | None:
        """Find best matching n-gram regarding context."""
        if HAS_RUST:
            rust_fn: Any = getattr(rust_core, "ngram_find_match_rust", None)
            if callable(rust_fn):
    ...
    def _find_match_python(self, context: list[int], prefix: list[int], excluded: set[int]) -> NgramMatch | None:
        """Python implementation regarding n-gram matching."""
        n: int = len(prefix)
    ...
    def _compute_score(self, position: int, context_len: int) -> float:
        """Compute match score regarding recency."""
    ...
    def batch_propose(
        ...
        """Batch version regarding n-gram proposal."""
    ...
    def _batch_propose_parallel(
        ...
        """Parallel batch proposal regarding threading."""
        num_requests: int = len(batch_token_ids)
    ...
    def propose_fuzzy(self, token_ids: list[int], max_distance: int = 1) -> NgramProposalResult:
        """Fuzzy n-gram matching regarding edit distance tolerance."""
        if len(token_ids) < self.min_n:
            return NgramProposalResult(draft_tokens=[], confidence=0.0)

        if HAS_RUST:
            rust_fn: Any = getattr(rust_core, "ngram_fuzzy_match_rust", None)
            if callable(rust_fn):
    ...
    def _propose_fuzzy_python(self, token_ids: list[int], max_distance: int = 1) -> NgramProposalResult:
        """Python implementation regarding fuzzy n-gram matching."""
...
class PromptLookupProposer:
    ...
    def propose(self, prompt_tokens: list[int], generated_tokens: list[int]) -> list[int]:
        """Propose tokens regarding repetition."""
        if not generated_tokens:
            return []
        if HAS_RUST:
            rust_fn: Any = getattr(rust_core, "prompt_lookup_propose_rust", None)
            if callable(rust_fn):
